...
const AddExpedition: React.FC<Props> = (({show, onHide}) => {
    ...
    const AddPointModal: React.FC<PropsModal> = (({show}) => {
        ...
        return (
            <Modal show={show} centered style={{fontFamily:'Rubik', zIndex:'10000', width: '90%'}} contentClassName='bg-dark text-light w-75' size={"sm"}>
                <Modal.Header>
    ...
    const onSubmit: SubmitHandler<ExpDTO> = async (data) => {
        try {
            const expeditionData: ExpeditionData = {
                ...
                starts_at: data.dateStart?.toISOString(),
                ends_at: data.dateFinish?.toISOString(),
                points: points,
            };
    ...
    return (
        <div>
            ...
            <Modal show={show} onHide={onHide} centered style={{fontFamily:'Rubik', zIndex:'2000'}} contentClassName='bg-dark text-light' size='xl'>
                ...
                <Modal.Body className='p-3 body-scroll'>
                        ...
                        >
                                ...
                                <Controller
                                    ...
                                    render={({ field: { value, onChange, ...restField } }) => (
                                        <DatePicker
                                            ...
                                            onChange={onChange}
                                            {...restField}
                                            status={errors.dateStart ? 'error' : ''}
                                            picker="date"
                                            popupStyle={{width: '100vw', position: 'absolute'}}
                            ...
                            <FormItem
                                validateStatus={errors.dateFinish ? 'error' : ''}
                                help={errors.dateFinish?.message}
                            >
                                <Controller
                                    name='dateFinish'
                                    control={control}
